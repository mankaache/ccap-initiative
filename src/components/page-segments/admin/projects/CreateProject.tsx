...
import { ArrowLeft, Briefcase, Save, X, Plus, Upload } from 'lucide-react';
...
export default function CreateProject() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast("Project created successfully!",{
      ...
      description: `${organizationData.ProjectTitle} has been added to the project portfolio.`,
    });
 ...
 const {t} = useTranslation();
  ...
  const addProgram = () => {
    //@ts-ignore
    if (newProgram && !organizationData.programs.includes(newProgram)) {
      //@ts-ignore
      setOrganizationData(prev => ({ ...prev, programs: [...prev.programs, newProgram] }));
      setNewProgram('');
    }
  };

  const removeProgram = (program: string) => {
    setOrganizationData(prev => ({ ...prev, programs: prev.programs.filter(p => p !== program) }));
  };
  const addRegion = () => {
    if (
      newRegion.trim() &&
      ...
      //@ts-ignore
      setOrganizationData((prevData) => ({...prevData,region: [...prevData.region, newRegion.trim()]}));
      setNewRegion("");
    }
  };
  ...
  const handleFileUpload = (files: FileList | null) => {
    if (files) {
      //@ts-ignore
      setOrganizationData(prev => ({ ...prev, images: [...prev.images, ...Array.from(files)] }));
    }
  };
